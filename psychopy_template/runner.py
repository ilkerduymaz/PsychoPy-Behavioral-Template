...
def runExperiment(exp=None, root_dir=None):
    # Ensure that relative paths start from the same directory as this script
    # _thisDir = os.path.dirname(os.path.abspath(__file__))
    _thisDir = (root_dir if root_dir is not None
                else os.path.dirname(os.path.abspath(__file__)))
    os.chdir(_thisDir)

    # Initialize Experiment object
    if exp is None:
        raise ValueError("Experiment object must be provided to runExperiment")
    win = exp.win
    ...
    for trial_index, thisTrial in enumerate(exp.trialhandler):
        ...
        while True:
            ...
            keys = event.getKeys()
            if len(keys) > 0:
                if "escape" in keys:  # Press ESC 2 times to abort
                    if endExpNow:
            ...
            if not continueRoutine or thisTrial.skip:
            ...
            # refresh the screen
            if continueRoutine:
                # don't flip if the routine is over or we'll get a blank screen
                win.flip()
